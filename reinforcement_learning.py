...
class RLAgent(Agent[S, A]):
    ...
    def act(self, env: Environment[S, A], observation: S, training: bool = False) -> A:
        ...
        if training and self.should_explore():
            ...
            maxq=-math.inf
            for act in actions:
                newq=self.compute_q(env,observation,act) #calculate the new q value with the current state and action
                if newq>maxq:  #if it is the max q value, then save it and its action
                    maxq=newq
                    bestact=act
...
class SARSALearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, next_action: Optional[A]):
        ...
        if next_state==None:
            #if it is terminal state, update the Q(state, action) with SARSA equation and applying self.Q[next_state][next_action]
            #  with 0 so the whole term that it is multiplied by will be 0
            self.Q[state][action]+=self.learning_rate*(reward-self.Q[state][action])  
        else:
            #update the Q(state, action) with SARSA equation
...
class QLearningAgent(RLAgent[S, A]):
    ...
    def compute_utility(self, env: Environment[S, A], state: S) -> float:
        ...
        #this function computes the q values of all actions at the current state and saves the max q value and return it by the end
        #of the function
        maxuti=-math.inf
        actions=self.actions
        for act in actions:
            newuti=self.compute_q(env,state,act)
            if newuti>maxuti:
                maxuti=newuti
        return maxuti
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        if done:
            #if next state  is terminal state, update the Q(state, action) with Q learning equation and applying 
            # self.Q[next_state][next_action] with 0 so the whole term that it is multiplied by will be 0
             self.Q[state][action]+=self.learning_rate*(reward-self.Q[state][action])
        else:
            #update the Q(state, action) with Q learning equation
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def __compute_q_from_features(self, features: Dict[str, float], action: A) -> float:
        ...
        #This function calculates the equation Q(s,a)=Sigma(wia*xi)
        sum=0
        for feature in features:
            sum+=self.weights[action][feature]*features[feature]
        return sum  
    ...
    def __compute_utility_from_features(self, features: Dict[str, float]) -> float:
        #TODO: Complete this function
        #This function gets the max Q value computed from features on the current state on all its actions
        maxuti=-math.inf
        actions=self.actions
        for act in actions:
            newuti=self.__compute_q_from_features(features,act)
            if newuti>maxuti:
                maxuti=newuti
        return maxuti
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        # If done is True, then next_state is a terminal state in which case, we consider the Q-value of next_state to be 0
        features=self.feature_extractor.extract_features(env,state) #get all features of current state
        currentq=self.compute_q(env,state,action)  #get teh q value of the current state and action Q(s,a)
        if done:
            #if next state  is terminal state, update the weights of action and feature with approximate Q learning equation and applying 
            # Q(sprime,aprime) with 0 so the whole term that it is multiplied by will be 0
            for feature in features:
                self.weights[action][feature]+=self.learning_rate*(reward-currentq)*features[feature]
        else:
            nextfeatures=self.feature_extractor.extract_features(env,next_state) #get all features of next state and use it to calculate max Q(sprime,aprime)
            maxuti=self.__compute_utility_from_features(nextfeatures)
            for feature in features:  #update the weights of action and feature with approximate Q learning equation
                self.weights[action][feature]+=self.learning_rate*(reward + self.discount_factor*maxuti -currentq)*features[feature]
